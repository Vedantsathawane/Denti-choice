const ClinicDoctorModel = require('../../models/clinic/clinicDoctorModel');
const { success, created, error, paginated } = require('../../utils/apiResponse');
const SocketService = require('../../services/socketService');

const ClinicDoctorController = {
  async getAll(req, res, next) {
    try {
      ...
      const doctors = await ClinicDoctorModel.findAll(clinicId, filters);

      if (limit) {
        const total = await ClinicDoctorModel.count(clinicId, filters);
  ...
  async getById(req, res, next) {
    try {
      ...
      const doctor = await ClinicDoctorModel.findById(clinicId, id);
  ...
  async create(req, res, next) {
    try {
      ...
      const id = await ClinicDoctorModel.create(clinicId, data);
      const doctor = await ClinicDoctorModel.findById(clinicId, id);
  ...
  async update(req, res, next) {
    try {
      ...
      await ClinicDoctorModel.update(clinicId, id, data);
      const updated = await ClinicDoctorModel.findById(clinicId, id);
      SocketService.emitDoctorsUpdated();
  ...
  async delete(req, res, next) {
    try {
      const clinicId = req.clinicId || 1;
      const id = req.body.id || req.params.id;
      const deleted = await ClinicDoctorModel.delete(clinicId, id);
...
module.exports = ClinicDoctorController;
